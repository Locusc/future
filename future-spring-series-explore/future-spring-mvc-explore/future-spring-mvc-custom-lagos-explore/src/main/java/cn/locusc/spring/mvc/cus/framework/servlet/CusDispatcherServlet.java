package cn.locusc.spring.mvc.cus.framework.servlet;
...
import cn.locusc.spring.mvc.cus.framework.pojo.Handler;
...
public class CusDispatcherServlet extends HttpServlet {
    ...
    private void initHandlerMapping() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            if(!aClass.isAnnotationPresent(CusController.class)) {continue;}


            String baseUrl = "";
            if(aClass.isAnnotationPresent(CusRequestMapping.class)) {
                CusRequestMapping annotation = aClass.getAnnotation(CusRequestMapping.class);
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(!method.isAnnotationPresent(CusRequestMapping.class)) {continue;}

                // 如果标识，就处理
                CusRequestMapping annotation = method.getAnnotation(CusRequestMapping.class);
    ...
    private void doAutoWired() {
        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < declaredFields.length; i++) {
                Field declaredField = declaredFields[i];   //  @CusAutowired  private IDemoService demoService;
                if(!declaredField.isAnnotationPresent(CusAutowired.class)) {
                    continue;
                }


                // 有该注解
                CusAutowired annotation = declaredField.getAnnotation(CusAutowired.class);
    ...
    private void doInstance()  {
        ...
        try{
            ...
            for (int i = 0; i < classNames.size(); i++) {
                ...
                if(aClass.isAnnotationPresent(CusController.class)) {
                ...
                }else if(aClass.isAnnotationPresent(CusService.class)) {
                    CusService annotation = aClass.getAnnotation(CusService.class);
                    //获取注解value值
